...
const ConfirmRidePopUp = (props) => {
    return (
        ...
        <h5 className='p-1 text-center absolute top-0 w-[93%]' onClick={()=> {
           ...
           <Link to='/captain-riding' className='w-full mt-5 flex justify-center bg-green-600 text-white font-semibold rounded-lg p-3'>Confirm</Link>
            <button onClick={()=> {
               props.setConfirmRidePopUpPanel(false);
               props.setRidePopUpPanel(false)
           }} className='w-full mt-1 bg-red-600 text-white font-semibold rounded-lg p-3'>Cancel</button>
           </div>
   </div>
    )
}
